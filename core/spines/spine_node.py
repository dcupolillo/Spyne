...
def get_path_to_root(
    ...
    while node_id in node_map and node_map[node_id].parent_id != 0:
        parent_id = node_map[node_id].parent_id
        path.append(node_id)
        node_id = parent_id
    path.append(node_id)  # add soma/root node
...
def path_distance(
    ...
    for i in range(path.index(stop_node_id)):
        n1 = node_map[path[i]]
        # If parent_id is -1 (soma), stop
        if n1.parent_id == -1 or n1.parent_id not in node_map:
            break
        n2 = node_map[n1.parent_id]
        dist += euclidean_distance((n1.x, n1.y, n1.z), (n2.x, n2.y, n2.z))
    return dist
...
def distance_along_neurite(
        ...
        node_id2: int) -> float:
    """
    Compute the symmetric distance along the neurite between two nodes
    by traversing up to their lowest common ancestor (LCA) and summing both paths.

    Parameters
    ----------
    nodes_list : list
        List of nodes, each with .id, .parent_id, .x, .y, .z
    node_id1 : int
    ...
    -------
    float
        The symmetric distance along the neurite between the two nodes.
    """
    node_map = {node.id: node for node in nodes_list}


    path1 = get_path_to_root(node_map, node_id1)
    path2 = get_path_to_root(node_map, node_id2)
