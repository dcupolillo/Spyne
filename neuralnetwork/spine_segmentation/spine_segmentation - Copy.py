...
def inference(
    ...
    #     spine_predictions = spine_predictions.squeeze()
    #     dendrite_predictions = dendrite_predictions.squeeze()
...
def process_single_prediction(
    ...
    mask_list = [spine for spine in spine_single_labels]

    # Initialize the dictionaries
    spines_dict = {
        'scanfield_index': roi.scanfield_index,
        'roi_index': roi.roi_index,
        'roi_uuid': roi.roi.roiuuid,
        'roi_z': roi.roi.z,
        'spine_n': np.arange(len(spine_single_labels)),
        'centroid_pix': np.asarray(centroids),
        'centroid_fov_deg': None,
        'mask': None,
        'ca_events': None,
        }
    ...
    intersection, intersection_indices = create_intersection_matrix(
        sd_masks, [mask for mask in spines_dict['mask']])
    ...
    skel_coords_fov = []
    for dendrite in dendrites_dicts[key].values():
    ...
    for dendrite_n, node in enumerate(skel_coords_fov):
        dendrites_dicts[key][dendrite_n]['skeleton_fov'] = node.tolist()
    ...
    for n, sd_mask in enumerate(sd_masks):
        if n not in existing_sd_indices:
            ...
            eroded_sd_mask = binary_erosion(sd_mask, structure=np.ones((3, 3)))
            ...
            if np.any(eroded_sd_mask):
                ...
                spines_dicts[key][new_spine_id] = new_spine_dict
                new_spine_id += 1

    # Convert list of masks to single label masks
    all_spine_labels = np.zeros_like(spine_labels)
    for n, spine in enumerate(spines_dicts[key].values()):
        all_spine_labels[spine['mask'] > 0] = n + 1
    ...
    cleaned_spines_dicts = {}
    cleaned_spines_dicts[key] = {}
    ...
    spines_dicts[key] = [{**spine, 'centroid_fov': centroid.tolist()}
                         for spine, centroid in zip(
                            cleaned_spines_dicts[key].values(),
                            centroids_fov)]
